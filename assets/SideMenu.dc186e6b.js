...
`;Ce(_e);j.render=ge;const Be=U({props:{title:null,navigatable:null},emits:["save","navigate"],setup(e,{emit:t}){const a=e,d=O(),c=()=>{const[,n]=location.pathname.match(/\/(playbook|contract)\//)||[];return n},l=(n,m)=>m.path.find(b=>b.equals(d.lastEditedStep))?B(_({},n),{style:"font-weight:bold"}):n,p=Y(a.navigatable.modules),u=G(()=>[{label:"Startseite",icon:"pi pi-angle-left",to:`/mitra-frontend/${d.entryPoint}`},{key:"1",label:a.title,icon:"pi pi-angle-down",to:`/mitra-frontend/${c()}/${a.navigatable.id}`,items:p.value.map(n=>l({label:n.title,command:()=>t("navigate",n)},n))},{label:"Speichern",icon:"pi pi-download",command:()=>t("save")}]);return(n,m)=>(s(),i(N(j),{model:N(u),"expanded-keys":{"1":!0},class:"w-64"},null,8,["model"]))}});export{Be as _,$ as a,se as b,ue as s};
